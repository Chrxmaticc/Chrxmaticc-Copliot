...
function init() {
  ...
  if (surpriseMode && Math.random() > 0.5) {
    setTimeout(function() {
      ...
      if (firstBubble) {
        firstBubble.innerHTML = '<strong>' + greetings[Math.floor(Math.random()*greetings.length)] + '</strong><br><br>I\'m Chrxmaticc Copilot. Offbrand. Hyper-intelligent. No subscription.<br><br>Type /help to see what I can do.';
      }
    }, 2400);
  }
}
...
function updateMiniAvatars() {
  ...
  Object.keys(savedChats).sort(function(a,b){return (savedChats[b].timestamp||0)-(savedChats[a].timestamp||0);}).slice(0,5).forEach(function(id) {
    ...
    d.innerHTML = '<img src="icon.png" style="width:100%;height:100%;object-fit:contain;border-radius:50%;" alt="Chat">';
    c.appendChild(d);
  });
}
...
function loadSavedChats() {
  ...
  var m = document.createElement('div'); m.className='chat-item'+(currentChatId==='main'?' active':'');
  m.dataset.chatId='main'; m.onclick=function(){loadChat('main');};
  ...
  Object.keys(savedChats).filter(function(id){return id!=='main';}).sort(function(a,b){return (savedChats[b].timestamp||0)-(savedChats[a].timestamp||0);}).forEach(function(id) {
    var c=savedChats[id]; var d=document.createElement('div'); d.className='chat-item'; d.dataset.chatId=id;
...
function toggleChatSearch() {
  ...
  if (chatSearchOpen) {
    bar.focus();
    toast('Search messages — type to find');
  } else {
    bar.value = '';
    clearHighlights();
  }
...
function searchChat() {
  ...
  document.querySelectorAll('.bubble-row .bubble').forEach(function(b) {
    ...
    if (text.indexOf(query) !== -1) {
      b.innerHTML = highlightText(b.textContent, query);
      b.closest('.bubble-row').scrollIntoView({ behavior: 'smooth', block: 'center' });
      count++;
    }
  });
  if (count === 0) toast('No matches found');
}

function highlightText(text, query) {
  var regex = new RegExp('(' + query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ')', 'gi');
  return text.replace(regex, '<mark style="background:rgba(255,200,50,0.3);border-radius:2px;padding:0 2px;">$1</mark>');
}
...
function setupSpeech() {
  ...
  recognition.onerror = function(event) {
    stopMicUI();
    if (event.error === 'not-allowed') toast('Microphone access denied');
    if (event.error === 'no-speech') toast('No speech detected');
  };
...
async function generateImage(prompt) {
  ...
  try {
    ...
    else { addError(data.error || 'Failed.'); }
...
function typeBubble(text, who, provider) {
  ...
  row.appendChild(ts);
  if (provider && who === 'ai') {
    var badge = document.createElement('div'); badge.className = 'provider-badge'; badge.textContent = provider;
...
function addMediaBubble(url, type, caption) {
  ...
  var img=document.createElement('img');img.src=url;img.alt=caption;img.loading='lazy';
...
function saveCurrentChat() {
  ...
  savedChats[currentChatId]={conversation:conversation.slice(-50),preview:conversation[conversation.length-1]?.content?.slice(0,50)||'',timestamp:Date.now()};
...
function triggerEasterEgg() {
  document.body.style.transform='rotate(0.3deg)'; setTimeout(function(){document.body.style.transform='';},500);
  launchConfetti(); toast('Easter egg found!');
}
function randomCompliment() {
  var c=['On fire today','Next level','Big brain energy','Legendary streak','The chrome demon approves'];
...
function launchConfetti() {
  ...
  function anim(){
    ctx.clearRect(0,0,confettiCanvas.width,confettiCanvas.height);var alive=false;
    particles.forEach(function(p){p.x+=p.vx;p.y+=p.vy;p.vy+=0.1;p.rot+=2;
      if(p.y<confettiCanvas.height+20){alive=true;ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.rot*Math.PI/180);ctx.fillStyle=p.color;ctx.fillRect(-p.size/2,-p.size/2,p.size,p.size);ctx.restore();}
    });
    if(alive)requestAnimationFrame(anim);else{confettiCanvas.style.display='none';confettiActive=false;}
  }
  anim();
}
...
// ═══════════════════════════════════════════
function toast(msg) {
  var t = document.createElement('div'); t.className = 'app-toast'; t.textContent = msg;
  document.body.appendChild(t);
  setTimeout(function() { if(t.parentNode) t.remove(); }, 2500);
}

// ═══════════════════════════════════════════
//  SETTINGS FUNCTIONS (shared with settings.html)
// ═══════════════════════════════════════════
