...
// ║  Media Support • File Reading • 5 Models ║
...
function loadSavedChats() {
  ...
  var m = document.createElement('div'); m.className='chat-item'+(currentChatId==='main'?' active':''); m.dataset.chatId='main';
...
function handleFileUpload(e) {
  ...
  if (filePreview) {
    if (file.type.startsWith('image/')) {
      ...
      filePreview.innerHTML = '<video src="'+URL.createObjectURL(file)+'" class="preview-vid" muted></video><div class="preview-info">'+file.name+'</div><button onclick="clearFile()">✕</button>';
      filePreview.classList.add('visible');
    } else {
...
function readFileContent(file) {
  return new Promise(function(resolve) {
    ...
    if (file.type.startsWith('image/') || file.type.startsWith('video/')) {
      resolve('[Media file: ' + file.name + ' (' + file.type + ', ' + (file.size/1024).toFixed(1) + 'KB)]');
      return;
    }
...
async function sendMessage() {
  ...
  if (text.toLowerCase().indexOf('/video') === 0) {
    var p = text.replace('/video','').trim();
    if (!p && !pendingFile) { addError('Usage: /video <description>'); return; }
    if (pendingFile) addMediaBubble(URL.createObjectURL(pendingFile), pendingFile.type.startsWith('video/')?'video':'image', pendingFile.name);
    generateVideo(p || 'a beautiful animation');
    inputEl.value = ''; clearFile(); return;
  }
  ...
  if (pendingFile) {
    if (pendingFile.type.startsWith('image/')) addMediaBubble(URL.createObjectURL(pendingFile), 'image', pendingFile.name);
    else if (pendingFile.type.startsWith('video/')) addMediaBubble(URL.createObjectURL(pendingFile), 'video', pendingFile.name);
  }
...
async function generateImage(prompt) {
  addHint('🎨 Copilot Artist 1.0 generating: "' + (prompt || 'image') + '"');
  try {
    var body = { prompt: prompt || 'a beautiful image', width: 512, height: 512 };
...
async function generateVideo(prompt) {
  addHint('🎬 Copilot Generator 1.0 generating: "' + (prompt || 'video') + '"');
  try {
    var body = { prompt: prompt || 'a beautiful animation' };
