...
// ║  Workflows • QA Strip • Memory • Toasts  ║
...
async function loadMemory() {
  initUserId();
  try { var res = await fetch('/api/memory',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'get',userId:memoryUserId})}); var data = await res.json(); userMemory.facts=data.facts||[]; userMemory.history=data.history||[]; } catch(e) {}
}
...
async function addFact(fact) {
  ...
  userMemory.facts.push(fact); if(userMemory.facts.length>50)userMemory.facts.shift();
...
function extractFacts(message) {
  ...
  for(var p of patterns){ var match=message.match(p.regex); if(match)addFact(p.prefix?p.prefix+' '+match[1].trim():match[1].trim()); }
}
function addToHistory(role,content) { userMemory.history.push({role:role,content:content}); if(userMemory.history.length>20)userMemory.history=userMemory.history.slice(-20); saveMemory(); }
function getMemoryContext() {
  var parts=[];
  if(userMemory.facts.length>0)parts.push('Facts: '+userMemory.facts.join('; '));
  if(userMemory.history.length>0){ var recent=userMemory.history.slice(-6); parts.push('Recent:\n'+recent.map(function(h){return(h.role==='user'?'User':'Copilot')+': '+(h.content||'').slice(0,150);}).join('\n')); }
  return parts.join('\n\n');
}
function checkMemoryEcho(message) { for(var f of userMemory.facts){ if(message.toLowerCase().indexOf(f.toLowerCase().replace(/^[^:]+:\s*/,''))!==-1)return f; } return null; }
...
function setPresence(state) {
  presenceState=state; if(!statusDot||!statusText)return;
  statusDot.className='status-dot '+state;
  var labels={online:'Online',thinking:'Thinking...',offline:'Offline',lurking:'Lurking...',locked:'Locked In',judging:'Judging...'};
  statusText.textContent=labels[state]||'Online'; clearTimeout(presenceTimer);
...
function init() {
  ...
  currentPersonality=localStorage.getItem('chrxmaticc_personality')||'sonnet';
  ...
  if(surpriseMode&&Math.random()>0.5){setTimeout(function(){var greetings=['Yo!','Hey!','Welcome back!','Sup!'];var firstBubble=document.querySelector('.bubble-row.ai .bubble');if(firstBubble)firstBubble.innerHTML='<strong>'+greetings[Math.floor(Math.random()*greetings.length)]+'</strong><br><br>I\'m Chrxmaticc Copilot.';},2400);}
...
function cyclePersonality() {
  var idx=PERSONALITIES.indexOf(currentPersonality); idx=(idx+1)%PERSONALITIES.length; currentPersonality=PERSONALITIES[idx];
  localStorage.setItem('chrxmaticc_personality',currentPersonality); updatePersonalityPill();
...
// ═══════════════════════════════════════════
//  ADAPTIVE MODE POPOVER
// ═══════════════════════════════════════════
var ALL_TOGGLES = {
  conversational: {
    code: [{id:'explain',label:'Explain Decisions'},{id:'compare',label:'Compare Approaches'},{id:'test',label:'Auto-Generate Tests'}],
    plan: [{id:'guided',label:'Guided Walkthrough'},{id:'beginner',label:'Beginner-Friendly'},{id:'examples',label:'Show Examples'}],
    review: [{id:'readability',label:'Readability Focus'},{id:'clarity',label:'Clarity Check'}],
    think: [{id:'deep',label:'Deep Analysis'},{id:'alternatives',label:'Explore Alternatives'}],
    surprise: [{id:'creative',label:'Creative Chaos'},{id:'wholesome',label:'Wholesome Mode'}]
  },
  sonnet: {
    code: [{id:'types',label:'Use TypeScript'},{id:'docs',label:'Generate Docs'},{id:'optimize',label:'Optimize Performance'}],
    plan: [{id:'architecture',label:'Architecture Focus'},{id:'deep',label:'Deep Analysis'},{id:'scalability',label:'Scalability Planning'}],
    review: [{id:'security',label:'Security Audit'},{id:'performance',label:'Performance Review'},{id:'patterns',label:'Design Patterns'}],
    think: [{id:'deep',label:'Deep Analysis'},{id:'tradeoffs',label:'Show Tradeoffs'}],
    surprise: [{id:'overengineer',label:'Over-Engineer'},{id:'polyglot',label:'Multi-Language'}]
  },
  vision: {
    code: [{id:'animate',label:'Add Animations'},{id:'theme',label:'Light+Dark Mode'},{id:'accessible',label:'WCAG Compliant'}],
    plan: [{id:'design',label:'Visual Design'},{id:'ux',label:'UX Focus'},{id:'moodboard',label:'Mood Board'}],
    review: [{id:'design',label:'Design Review'},{id:'a11y',label:'Accessibility Audit'}],
    think: [{id:'visual',label:'Visual Thinking'},{id:'mood',label:'Mood Analysis'}],
    surprise: [{id:'experimental',label:'Experimental Design'},{id:'glitch',label:'Glitch Mode'}]
  },
  intermediate: {
    code: [{id:'minimal',label:'Minimal Code'},{id:'reuse',label:'Prefer Libraries'},{id:'ship',label:'Ship-Ready Output'}],
    plan: [{id:'fast',label:'Fast Planning'},{id:'practical',label:'Practical Only'}],
    review: [{id:'practicality',label:'Practicality Check'},{id:'simplicity',label:'Simplify Code'}],
    think: [{id:'efficient',label:'Efficient Analysis'}],
    surprise: [{id:'useful',label:'Useful Chaos'},{id:'shortcuts',label:'Clever Shortcuts'}]
  },
  speed: {
    code: [{id:'inline',label:'Single File'},{id:'nocomments',label:'No Comments'},{id:'instant',label:'Instant Output'}],
    plan: [{id:'rapid',label:'Rapid Plan'},{id:'bullet',label:'Bullet Points Only'}],
    review: [{id:'quick',label:'Quick Review'},{id:'critical',label:'Critical Only'}],
    think: [{id:'snap',label:'Snap Judgment'}],
    surprise: [{id:'random',label:'Pure Random'},{id:'speedrun',label:'Speedrun Mode'}]
  }
};

function getToggles() {
  var p = ALL_TOGGLES[currentPersonality] || ALL_TOGGLES.sonnet;
  var w = p[currentWorkflow] || p.code || [];
  return w;
}
...
function toggleModePopover() {
  ...
  popover.style.cssText = 'position:fixed;bottom:110px;left:120px;z-index:400;background:var(--card-bg);border:1px solid var(--border);border-radius:16px;padding:16px;min-width:220px;box-shadow:0 8px 40px rgba(0,0,0,0.5);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);';

  var html = '<div style="font-size:12px;font-weight:700;color:var(--accent);margin-bottom:10px;">'+PERSONALITY_LABELS[currentPersonality]+' • '+WORKFLOW_LABELS[currentWorkflow]+'</div>';
  ...
  if (toggles.length > 0) {
    html += '<div style="font-size:11px;color:var(--muted);margin-bottom:4px;">Options</div>';
...
//  PLAN QA STRIP
// ═══════════════════════════════════════════
function showQAStrip(data) {
  var strip = document.getElementById('qaStrip');
  if (!strip) return;
  strip.innerHTML = '<span class="qa-strip-label">'+data.question+'</span>';
...
function acceptCode(btn) {
  ...
  btn.textContent='Accepted';btn.style.borderColor='#30d158';
  phaseToast('accepted. you have good taste.','accept');
}

function rejectCode(btn) {
  rejectCount++; if(rejectCount>=5)earnBadge('perfectionist');
...
function showTokenToast(token) {
  ...
  toastEl.innerHTML='<div style="color:var(--accent);font-weight:700;margin-bottom:6px;">Your New Token</div><div style="background:rgba(212,165,116,0.08);border:1px solid rgba(212,165,116,0.2);border-radius:8px;padding:8px;font-family:monospace;font-size:11px;word-break:break-all;color:var(--text);margin-bottom:10px;">'+token+'</div><div style="color:#ff9f0a;font-size:10px;margin-bottom:8px;">Save this. It rotates every login.</div><button onclick="this.parentElement.remove()" style="background:var(--panel);border:1px solid var(--border);color:var(--text);padding:6px 16px;border-radius:8px;cursor:pointer;font-size:11px;font-family:inherit;">Close</button>';
  document.body.appendChild(toastEl);setTimeout(function(){if(toastEl.parentElement)toastEl.remove();},30000);
}
