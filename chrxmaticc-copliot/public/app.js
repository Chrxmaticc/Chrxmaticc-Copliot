...
function handleFileUpload(e) {
  ...
  pendingFile = file;
  if (filePreview) {
    if (file.type.startsWith('image/')) { filePreview.innerHTML = '<img src="'+URL.createObjectURL(file)+'" class="preview-img"><button onclick="clearFile()">✕</button>'; filePreview.classList.add('visible'); }
    else if (file.type.startsWith('video/')) { filePreview.innerHTML = '<video src="'+URL.createObjectURL(file)+'" controls class="preview-vid"></video><button onclick="clearFile()">✕</button>'; filePreview.classList.add('visible'); }
    else { filePreview.innerHTML = '<div class="preview-file">📎 '+file.name+'</div><button onclick="clearFile()">✕</button>'; filePreview.classList.add('visible'); }
  }
}
...
async function sendMessage() {
  ...
  // /image command
  if (text.toLowerCase().indexOf('/image') === 0) { var p = text.replace('/image','').trim(); if (!p && !pendingFile) { addError('Usage: /image <description>'); return; } generateImage(p); inputEl.value = ''; clearFile(); return; }
  // /video command
  if (text.toLowerCase().indexOf('/video') === 0) { var p = text.replace('/video','').trim(); if (!p && !pendingFile) { addError('Usage: /video <description>'); return; } generateVideo(p); inputEl.value = ''; clearFile(); return; }

  var displayText = text || (pendingFile ? '📎 ' + pendingFile.name : '');
  addBubble(displayText, 'user');
  if (pendingFile) { addMediaBubble(URL.createObjectURL(pendingFile), pendingFile.type.startsWith('video/') ? 'video' : 'image', pendingFile.name); }

  conversation.push({ role:'user', content:displayText }); inputEl.value = ''; inputEl.style.height = 'auto';
  if (sendBtn) sendBtn.disabled = true; if (typingEl) typingEl.classList.add('visible');
  setStatus('thinking'); if (messagesEl) messagesEl.scrollTop = messagesEl.scrollHeight;
  messageCount++;

  try {
    var body = { message: text, personality: currentPersonality };
    if (pendingFile) { body.fileContext = 'File: ' + pendingFile.name + ' (' + pendingFile.type + ', ' + (pendingFile.size/1024).toFixed(1) + 'KB)'; }
...
async function generateImage(prompt) {
  addHint('🎨 Copilot Artist 1.0 generating: "' + (prompt || 'from reference') + '"');
  try {
    var body = { prompt: prompt, width: 512, height: 512 };
    if (pendingFile) body.referenceUrl = URL.createObjectURL(pendingFile);
    var res = await fetch('/api/image', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) });
...
async function generateVideo(prompt) {
  addHint('🎬 Copilot Generator 1.0 generating: "' + (prompt || 'from reference') + '"');
  try {
    var body = { prompt: prompt };
    if (pendingFile) body.referenceUrl = URL.createObjectURL(pendingFile);
    var res = await fetch('/api/video', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) });
    var data = await res.json();
    if (data.success && data.frames) { data.frames.forEach(function(url,i) { setTimeout(function(){ addMediaBubble(url, 'image', prompt+' (frame '+(i+1)+')'); }, i*500); }); }
    else { addError(data.error || 'Generation failed.'); }
  } catch(e) { addError('Video service offline.'); }
}
